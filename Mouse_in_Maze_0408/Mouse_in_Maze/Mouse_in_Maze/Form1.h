		...
		void GoMaze() {
			...
			while (top != -1) {
				...
				while (dir <= 7) {
					...
					break;
					if (w[u][v] == 0 && mark[u][v] == 0) {

						mark[u][v] = 1;
						step.x = u; step.y = v;
						//step.dir = static_cast<directions>(dir);
						step.dir = directions(dir);
						listBox_ErrorBox->Items->Add("(" + Convert::ToString(step.x) + "," + Convert::ToString(step.y) + "," + Convert::ToString(step.dir) + ")");
						push(step);
			...
			listBox_ErrorBox->Items->Add("迷宮無解");
			listBox_ErrorBox->Items->Add(top);
			//收回動態空間
			DeleteArray();
